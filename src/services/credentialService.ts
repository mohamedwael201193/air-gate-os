...
export class CredentialService {
  async issueCredential(
    ...
    try {
      ...
      // Ensure credentialSubject.id is present
      const credentialSubject = {
        ...data,
        id: data.id || getSubjectId(),
      };

      const result = await airIssue(credentialId, credentialSubject);
  ...
  async verifyCredential(
    ...
    try {
      ...
      const result = await airVerify(
        programId,
        import.meta.env.VITE_REDIRECT_URL || window.location.origin + "/profile"
      );
      ...
      const record: VerificationRecord = {
        ...
        proofId:
          ...
          `tx_${Date.now()}`, // Use txHash as proof
  ...
  getStatistics() {
    ...
    return {
      ...
      activeCredentials: credentials.filter((c) => c.status === "active")
        .length,
      totalVerifications: verifications.length,
      successfulVerifications: verifications.filter(
        (v) => v.status === "success"
      ).length,
      successRate:
